...
public final class Funcion{
	...
	public static double activacion(byte funcion, double potencial){
		...
		else if(funcion == 2){	/** Funcion de activacion(escalonada): Escalon Binario */
			if(potencial > 0.0)
				activacion = 1.0;
			else
				activacion = 0.0;
		}
		else if(funcion == 3){	/** Funcion de activacion(escalonada): Escalon Bipolar */
			if(potencial > 0.0)
				activacion = 1.0;
			else
				activacion = -1.0;
		}
		else if(funcion == 4){	/** Funcion de activacion(sigmoidea): Logaritmo Sigmoidal */
			activacion = 1.0 / (1.0 + (Math.pow(Math.E, -potencial)));
		}
		else if(funcion == 5){	/** Funcion de activacion(sigmoidea): Tangente Sigmoidal */
			activacion = (2.0 / (1.0 + (Math.pow(Math.E, -potencial)))) - 1.0;
		}
		else if(funcion == 6){	/** Funcion de activacion(sigmoidea): Tangente Hiperbolica */
